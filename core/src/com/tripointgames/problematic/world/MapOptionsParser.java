...
public class MapOptionsParser {
	...
	public void parse() {
		/*
		 * ENTITIES LAYER Load in all entities form the Entities layer. This
		 * basically checks the layer for map objects, and collects data from
		 * their properties, which are set in Tiled (map editor).
		 */

		// Get the layer if it exists, or return if it was not found.
		MapLayer entityLayer = this.map.getLayers().get("Entities");
		if (entityLayer == null) {
		...
		while (mapObjects.hasNext()) {
			...
			if (type.equalsIgnoreCase("player")) {
				...
				playerSpawnLocation = new Vector2(object.getProperties().get(
						"x", Float.class), object.getProperties().get("y",
						Float.class));
			} else {
				// The entity does not exist, print a warning and move on.
				System.err
						.println("Object found in the entity layer but is not a recognized entity. (" + type + ")");
			}
		}
	}
